...
async function loadAllData() {
  showLoading();
  await Promise.all([loadFlow()]);
  renderAll();
}

function showLoading() {
  document.getElementById("pkg-list").innerHTML = `<div class="loading-row"><div class="spinner"></div>加载中...</div>`;
  document.getElementById("speed-area").innerHTML = `<div class="empty-row">速率查询已关闭</div>`;
  document.getElementById("biz-area").innerHTML = `<div class="empty-row">套餐查询已关闭</div>`;
}
...
function renderFlow() {
  ...
  list.forEach(item => {
    ...
    <div class="pkg-item">
      ...
      <div class="pkg-nums">
        <span>剩余 ${fmt(remain)} MB</span>
        <span>${fmt(used)} / ${fmt(total)} MB</span>
      </div>
    </div>`;
  });
  ...
  document.getElementById("s-remain").textContent = `${fmt(allRemain)} MB`;
  document.getElementById("s-used").textContent = `${fmt(totalUsed)} MB`;
  document.getElementById("s-total").textContent = `${fmt(totalTotal)} MB`;
  document.getElementById("s-pct").textContent = `${allPct}%`;
  document.getElementById("main-bar").style.width = `${allPct}%`;
  document.getElementById("bar-used-lbl").textContent = `已用 ${fmt(totalUsed)} MB`;
  document.getElementById("bar-total-lbl").textContent = `共 ${fmt(totalTotal)} MB`;
